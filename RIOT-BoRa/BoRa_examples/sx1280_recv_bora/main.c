...
static void _event_cb(netdev_t *dev, netdev_event_t event)
{
    ...
    if (event == NETDEV_EVENT_ISR) {
        ...
        case NETDEV_EVENT_TX_COMPLETE:
            ...
            // sx1280_read_register( (&(dev->ral))->context, addr, read_buffer, 1 );
            // printf("pre:sync:0x%x\n", read_buffer[0]);
            ...
            // sx1280_read_register( (&(dev->ral))->context, addr, read_buffer, 6 );
            // printf("after_add:0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n", read_buffer[0],read_buffer[1],read_buffer[2],read_buffer[3],read_buffer[4],read_buffer[5]);

...
/*
 * We use the symbol number calculation method from jkadbear/gr-lora.
 * However, this is a LoRa receiver fro SX127x, but the SX128x use different symbol number calculation for SF <= 6 (Sec. 7.4.4.1 in SX1280 datasheet)
 * Further exploration is needed when SF <= 6
*/
...
int encode(uint8_t *bytes, int length, uint16_t *freq_symbols)
{
    ...
    uint16_t sym_num = calc_sym_num(length);  // sym_num = ceil((2 * payload_length - (d_sf - 2)) / d_sf) * 4
    uint16_t nibble_num = d_sf - 2 + (sym_num - 8) / (d_cr + 4) * (d_sf - 2 * d_ldr);
    uint16_t redundant_num = ceil(((double)nibble_num - (double)(2 * idx)) / 2);

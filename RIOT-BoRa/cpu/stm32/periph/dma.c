...
#if BORA
// For ADC buffer access control 
extern sema_t buf_sema1;
extern sema_t buf_sema2;
extern int sem1_cnt;
extern int sem2_cnt;
extern volatile uint32_t sem1_time_stamp, sem2_time_stamp;
STM32_DMA_Stream_Type *bora_dma_stream;
#endif
...
#if BORA
    bora_dma_stream = dma_ctx[adc_dma1].stream;
#endif
}
               ...
               uint8_t width, bool inc_periph)
{
    ...
    uint32_t cr_settings = (chan & 0xF) << DMA_SxCR_CHSEL_Pos |
                           ...
                           DMA_SxCR_TEIE;
#if BORA
    if (mode == 0 && dma == adc_dma1)
    {
        /* BoRa: For the DMA for ADC, we set the highest priority */
        cr_settings |= (0x3UL << DMA_SxCR_PL_Pos);
        /* BoRa: We use the double-buffer mode */
        cr_settings |= DMA_SxCR_DBM;
    }
#endif
    /* Configure FIFO */
...
#if BORA
    /* BoRa: we use double-buffer mode */
    if (dma == adc_dma1) {
        stream->M0AR = (uint32_t)mem;
        stream->M1AR = ((uint32_t)mem) + ADC_DMA_TRANSFRE_NUM * 4;
    }
    else {
...
{
#if BORA
    if (bora_dma_stream->CR & DMA_SxCR_CT_Msk) 
    {
        /* Memeory address 0 is updated */
        sema_post(&buf_sema1);
        sem1_time_stamp = ztimer_now(ZTIMER_USEC);
        sem1_cnt++;
    }
    else {
        sema_post(&buf_sema2);
        sem2_time_stamp = ztimer_now(ZTIMER_USEC);
        sem2_cnt++;
    }
#endif
    dma_isr_handler(3);

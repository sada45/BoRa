                    ...
                    sdmmc_resp_t resp_type, uint32_t *resp)
{
    ...
    if (res) {
        DEBUG("[329 sdmmc] command failed with error %d\n", res);
        return res;
    }
...
int sdmmc_card_init(sdmmc_dev_t *dev)
{
    ...
    if (res) {
        DEBUG("[364 sdmmc] command failed with error %d\n", res);
        goto out;
    }
    ...
    do {
        ...
        if (res == -ETIMEDOUT) {
            ...
            DEBUG("[sdmmc] 473 command failed with error %d\n", res);
            res = -EIO;
            goto out;
        }
    ...
    if (res) {
        DEBUG("[sdmmc546] command failed with error %d\n", res);
        goto out;
    }
    ...
    if (res) {
        DEBUG("[561 sdmmc] command failed with error %d\n", res);
        goto out;
    }
    ...
    if (dev->type != SDMMC_CARD_TYPE_MMC) {
        ...
        res = _send_cmd(dev, SDMMC_CMD16, SDMMC_SDHC_BLOCK_SIZE,
                        SDMMC_R1, response);
        if (res) {
            DEBUG("[578 sdmmc] command failed with error %d\n", res);
            goto out;
        }
        ...
        if (dev->bus_width != SDMMC_BUS_WIDTH_1BIT) {
            ...
            res = _send_acmd(dev, SDMMC_ACMD6,
                             ...
                             SDMMC_R1, response);
            if (res) {
                DEBUG("[600 sdmmc] command failed with error %d\n", res);
                goto out;
            }
        ...
        if (IS_USED(MODULE_PERIPH_SDMMC_HS) && dev->scr.SD_SPEC) {
            ...
            if (sw_status->group1_selected == SDMMC_CMD6_FUNC_GROUP1_HS) {
                ...
                printf("set 25M\n");
#if BORA
                /* BoRa: lower clk speed to be more stable */
        ...
        DEBUG("bus width inti= 123123_%d\n", dev->bus_width);
                      ...
                      sdmmc_resp_t resp_type, uint32_t *resp)
{
    ...
    DEBUG("[sdmmc acmd] %s dev=%p cmd_idx=%u arg=%"PRIu32" resp_type=%u resp=%p\n",
          __func__, dev, cmd_idx, arg, resp_type, resp);
    ...
    if (res) {
        DEBUG("[954 sdmmc] command failed with error %d\n", res);
        return res;
    }
                 ...
                 const void *data_wr, void *data_rd, uint16_t *done)
{
    ...
    if (cmd_idx & SDMMC_ACMD_PREFIX) {
        res = _send_cmd(dev, SDMMC_CMD55, SDMMC_CMD_ARG_RCA(dev->rca),
                        SDMMC_R1, &response);
        if (res) {
            DEBUG("[1039 sdmmc] command failed with error %d\n", res);
            goto out;
        }
    }
    ...
    if (block_num > 1) {
        if (IS_USED(MODULE_PERIPH_SDMMC_AUTO_CMD12)) {
            ...
            res = _send_cmd(dev, SDMMC_CMD12, SDMMC_CMD_NO_ARG, SDMMC_R1B, &response);
            /* BoRa: native RIOT OS only send the CMD12 once and does not check the result,
             * Sometimes the SD card is busy on the transmission and doest not reply
             * We retry the CMD12 so the card can normally respond.
             * Remember to submit a PR to RIOT OS latter...
             */
            while (res != 0) {
                ztimer_sleep(ZTIMER_USEC, 2);
                res = _send_cmd(dev, SDMMC_CMD12, SDMMC_CMD_NO_ARG, SDMMC_R1B, &response);
                DEBUG("waiting to stop trans \n");
            }
        }
    }
...
static int _select_deselect(sdmmc_dev_t *dev, bool select)
{
    ...
    if (res) {
        DEBUG("[1157 sdmmc] command failed with error %d\n", res);
        return res;
    }
...
static int _get_status(sdmmc_dev_t *dev, sdmmc_card_status_t *cs)
{
    ...
    /* card must have a valid RCA */
    assert(dev->rca);
    int res = _send_cmd(dev, SDMMC_CMD13, SDMMC_CMD_ARG_RCA(dev->rca),
                  SDMMC_R1, &cs->value);
    if (res) {
...
static inline int _wait_for_ready(sdmmc_dev_t *dev, uint32_t timeout_ms)
{
    ...
    do {
        ...
        // cs_val[cs_val_i++] = cs.value;
        /* BoRa: We also check the SD care is in tran mode */
        if ((SDMMC_CARD_STATUS_CURRENT_STATE(cs.value) == 4) && (cs.value & SDMMC_CARD_STATUS_READY_FOR_DATA) ) {
            // for (int i = 0; i < cs_val_i; i++) {
            //     printf("cs val i = %ld\n", cs_val[i]);
            // }
            return 0;
        }
...
static int _read_cid(sdmmc_dev_t *dev, uint8_t cmd)
{
    ...
    if (res) {
        DEBUG("[1327 sdmmc] command failed with error %d\n", res);
        return res;
    }
...
static int _read_csd(sdmmc_dev_t *dev)
{
    ...
    if (res) {
        DEBUG("[1401 sdmmc] command failed with error %d\n", res);
        return res;
    }
...
static int _read_scr(sdmmc_dev_t *dev)
{
    ...
    if (res) {
        DEBUG("[1452 sdmmc] command failed with error %d\n", res);
        return res;
    }
...
static int _read_ext_csd(sdmmc_dev_t *dev)
{
    ...
    if (res) {
        DEBUG("[1526 sdmmc] command failed with error %d\n", res);
...
static int _write_ext_csd(sdmmc_dev_t *dev, uint8_t index, uint8_t value)
{
    ...
    if ((res = _send_cmd(dev, SDMMC_CMD6, arg, SDMMC_R1B, &response))) {
        DEBUG("[1567 sdmmc] command failed with error %d\n", res);
        return res;
    }

...
int main() {
	...
	debug_uart_init(0);
	debug_printf("\n\n****UBERTOOTH BOOT****\n%s\n", compile_info);
	for (i = 0; i < chirp_num; i++) {
		start_freq[i] = 0;
	}
	start_freq[PREAMBLE_LEN] = 5;
	start_freq[PREAMBLE_LEN+1] = 5;

	start_freq[PREAMBLE_LEN+2] = BLE_BBIN_NUM;
	start_freq[PREAMBLE_LEN+3] = BLE_BBIN_NUM;
	start_freq[PREAMBLE_LEN+4] = BLE_BBIN_NUM + 2;  // Reserved for the last one-quater SFD
	for (int i = 0; i < LORA_SYMBOLS; ++i)
	{
		start_freq[PREAMBLE_LEN + 5 + i] = i % BLE_BBIN_NUM;
	}
	wait(1);
	clkn_start();
	bora_periodic_tx();
